...
const toDelete = (data: SourceItem) => {
  modal.confirm({
    ...
    async onOk () {
      ...
      const [error] = await http.del(`${TESTER}/target/${data.targetId}/${data.targetType?.value}/dataset`, params, { dataType: true });
      loading.value = false;
      if (error) {
        return;
      }
  ...
  modal.confirm({
    ...
    async onOk () {
      ...
      for (let i = 0; i < num; i++) {
        ...
        if (data) {
          const _promise = http.del(`${TESTER}/target/${data.targetId}/${data.targetType?.value}/dataset`, params, { dataType: true, silence: true });
