...
const allFuncNames = computed(() => {
  return allFunction.value.map((i: any) => i.name);
});
        ...
        <div
          ...
          <div class="flex-1 w-full overflow-y-auto min-h-0">
            <ServerPath
              ...
              @sendRequest="() => sendRequest(state, setting.value, currentServer.value || { url: '' }, apiUri.value || '', apiMethod.value, contentType.value, saveParams.value, requestBodyRef, authorizationRef, assertFormRef, isUnarchivedApi.value, ws, allFuncNames.value, handleHttpResponse, () => {})"
              @save="save"
