...
const dataSource = computed(() => {
  ...
  const type = deepCloneValue.type;
  if (type === 'object') {
    return objectToArray(deepCloneValue.properties);
  }

  if (type === 'array') {
    return itemsToArray(deepCloneValue);
  }

  const example = getPropertyExample(deepCloneValue);
  return { 'x-xc-example': example, ...deepCloneValue };
});
</script>
