      ...
      <div class="space-y-1.5 pl-3 mb-2">
        ...
        <div>
          ...
          <div class="pl-5 space-y-2">
            <div
              v-for="_scope in scopes"
              :key="_scope.id"
              class="flex items-center space-x-1.5">
              <div class="flex-shrink-0 w-1 h-1 bg-black-log-bg rounded"></div>
              <Tag class="flex-shrink-0 max-w-25 truncate">{{ _scope.value }}</Tag>
              <div class="truncate" :title="scopesMap[item][_scope.value]">{{ scopesMap[item][_scope.value] }}</div>
            </div>
          </div>
        </div>
      </div>
