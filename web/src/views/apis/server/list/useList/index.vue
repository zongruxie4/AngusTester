...
const toDelete = (data: SourceItem) => {
  modal.confirm({
    ...
    async onOk () {
      ...
      const [error] = await paramTarget.deleteVariable(data.targetId, data.targetType?.value, params);
      loading.value = false;
      if (error) {
        return;
      }
  ...
  modal.confirm({
    ...
    async onOk () {
      ...
      for (let i = 0; i < num; i++) {
        ...
        if (data) {
          const _promise = paramTarget.deleteVariable(data.targetId, data.targetType?.value, params);
