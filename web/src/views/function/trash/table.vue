...
const recoverHandler = async (data: TrashItem) => {
  ...
  const [error] = await http.patch(`${TESTER}/func/trash/${data.id}/back`);
  if (error) {
...
const deleteHandler = async (data: TrashItem) => {
  emit('update:spinning', true);
  const [error] = await http.del(`${TESTER}/func/trash/${data.id}`);
  if (error) {
    emit('update:spinning', false);
    return;
  }
...
const loadData = async () => {
  ...
  const [error, res] = await http.get(`${TESTER}/func/trash/search`, params);
  loaded.value = true;
  emit('update:spinning', false);
  if (error) {
    return;
