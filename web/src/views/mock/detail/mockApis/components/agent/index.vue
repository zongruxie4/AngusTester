...
const agentOptions = ref([{
  label: t('mock.mockApisComp.agent.noProxy'),
  value: 'NO_PROXY',
  enabled: false,
  key: 'noProxy',
  url: ''
}, {
  label: t('mock.mockApisComp.agent.clientProxy'),
  value: 'CLIENT_PROXY',
  enabled: false,
  key: 'clientProxy',
  url: ''
}, {
  label: t('mock.mockApisComp.agent.serverProxy'),
  value: 'SERVER_PROXY',
  enabled: false,
  key: 'serverProxy',
  url: ''
...
const tipsMap = {
  NO_PROXY: t('mock.mockApisComp.agent.descriptions.noProxy'),
  CLIENT_PROXY: t('mock.mockApisComp.agent.descriptions.clientProxy'),
  SERVER_PROXY: t('mock.mockApisComp.agent.descriptions.serverProxy'),
  CLOUD_PROXY: t('mock.mockApisComp.agent.descriptions.cloudProxy')
};
...
<template>
  <div class="text-3">
    <div class="flex items-center space-x-2">
      <div class="font-semibold">{{ t('mock.mockApisComp.agent.proxyConfig') }}</div>
      <div v-show="currentProxy !== 'NO_PROXY'" class="flex items-center">
        <template v-if="status === 'fail'">
          ...
          {{ t('mock.mockApisComp.agent.notConnected') }}
        </template>
        <template v-if="status === 'success'">
          <Icon icon="icon-duihao" class="text-execute-res4" />
          {{ t('mock.mockApisComp.agent.connected') }}
        </template>
      </div>
    </div>
    ...
    <template v-if="agent !== 'NO_PROXY'">
      ...
      <div v-show="currentProxyUrl && status === 'fail'" class="mt-3 text-status-orange">{{ t('mock.mockApisComp.agent.notifications.connectionFailed') }}</div>
    </template>
  </div>
</template>
<style scoped>
