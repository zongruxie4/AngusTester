...
onMounted(() => {
  watch(() => props.analysisInfo, (newValue) => {
    if (newValue) {
      ...
      totalValue.value.value0 = [{ name: t('taskAnalysis.detail.progress.unfinished'), value: totalNum - completedNum }, { name: t('taskAnalysis.detail.progress.completed'), value: completedNum }];
      totalValue.value.value1 = [{ name: t('taskAnalysis.detail.progress.unfinished'), value: evalWorkload - completedWorkload }, { name: t('taskAnalysis.detail.progress.completed'), value: completedWorkload }];
      ...
      if (newValue?.containsUserAnalysis) {
        ...
        Object.keys(sourceData).forEach(userId => {
          ...
          const chartData = {
            value0: [{ name: t('taskAnalysis.detail.progress.unfinished'), value: totalNum - completedNum }, { name: t('taskAnalysis.detail.progress.completed'), value: completedNum }],
            value1: [{ name: t('taskAnalysis.detail.progress.unfinished'), value: evalWorkload - completedWorkload }, { name: t('taskAnalysis.detail.progress.completed'), value: completedWorkload }],
