...
<template>
  <div>
    <h1 class="text-theme-title font-medium mb-5">
      <span id="a13" class="text-4 text-theme-title font-medium">{{ t('reportPreview.serial.4') }}<em class="inline-block w-0.25"></em>{{ t('reportPreview.execPerf.log.title') }}</span>
    </h1>

    <div class="mb-7">
      <h2 class="flex items-center space-x-2.5 text-3.5 mb-3.5 text-theme-title">
        <span id="a14">4.1<em class="inline-block w-3.5"></em>{{ t('reportPreview.execPerf.log.schedulingLog.title') }}</span>
      </h2>
      <div class="flex items-center mb-2">
        ...
        <div class="flex items-center">
          <div class="mr-0.5">{{ t('reportPreview.execPerf.log.schedulingLog.fields.processExitCode') }}：</div>
          <div v-if="nodeIp">{{ schedulingLog?.exitCode }}</div>
          <div v-else>--</div>
        </div>
      </div>
      <div
        class="text-0 text-white bg-slate-600 px-2.5 py-2 rounded border border-solid border-theme-text-box">
        <div
    ...
    <div class="mb-7">
      <h2 class="flex items-center space-x-2.5 text-3.5 mb-3.5 text-theme-title">
        <span id="a15">4.2<em class="inline-block w-3.5"></em>{{ t('reportPreview.execPerf.log.samplingLog.title') }}</span>
      </h2>
      <div class="flex items-center mb-2">
        <div class="flex items-start">
          <div class="mr-0.5 flex-shrink-0">{{ t('reportPreview.execPerf.log.samplingLog.fields.file') }}：</div>
          <div v-if="!execLogErr && execLogPath" class="whitespace-pre-wrap break-all">
            {{ execLogPath }}
          </div>
          <div v-else class="text-rule whitespace-pre-wrap break-all">{{ errorText || `${t('reportPreview.execPerf.log.samplingLog.messages.noProxyInfo')}http://${nodeIp}:6807/actuator/runner/log/${props.execInfo?.id}` }}</div>
        </div>
      </div>
      <div
