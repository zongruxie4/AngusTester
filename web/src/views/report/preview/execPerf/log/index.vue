...
<template>
  <div>
    <h1 class="text-theme-title font-medium mb-5">
      <span id="a13" class="text-4 text-theme-title font-medium">{{ t('reportPreview.serial.4') }}<em class="inline-block w-0.25"></em>日志信息</span>
    </h1>

    <div class="mb-7">
      <h2 class="flex items-center space-x-2.5 text-3.5 mb-3.5 text-theme-title">
        <span id="a14">4.1<em class="inline-block w-3.5"></em>执行调度信息</span>
      </h2>
    ...
    <div class="mb-7">
      <h2 class="flex items-center space-x-2.5 text-3.5 mb-3.5 text-theme-title">
        <span id="a15">4.2<em class="inline-block w-3.5"></em>执行采样信息</span>
      </h2>
      <div class="flex items-center mb-2">
        <div class="flex items-start">
          ...
          <div v-else class="text-rule whitespace-pre-wrap break-all">{{ errorText || `无访问代理信息，连接失败地址：http://${nodeIp}:6807/actuator/runner/log/${props.execInfo?.id}` }}</div>
        </div>
      </div>
      <div
