...
const refTaskInfos = computed(() => {
  return props.dataSource?.content?.task?.refTaskInfos || [];
});
...
<template>
  <div>
    ...
    <div v-else class="content-text-container">{{ t('reportPreview.task.assocTask.noData') }}</div>
  </div>
</template>
