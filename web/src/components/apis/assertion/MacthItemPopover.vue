 ...
 * @param rawXmlData - Raw XML string to format
 * @returns Formatted XML string with proper indentation
 */
const formatXmlWithIndentation = (rawXmlData: string): string => {
  return beautify.html(rawXmlData, {
...
// Configuration for match item examples
const matchItemExamplesConfig = [
  {
    key: 'regexp',
    ...
    <template #content>
      <div style="max-height: 60vh; overflow: auto;" class="w-196 space-y-2 text-3 text-theme-content">
        ...
        <ol class="pl-4 space-y-2">
          <li
            v-for="example in matchItemExamplesConfig"
            :key="example.key"
