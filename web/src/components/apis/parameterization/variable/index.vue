...
const selectedVariablesOk = async (data: VariableItem[]) => {
  ...
  loading.value = true;
  const [error] = await http.post(`${TESTER}/target/${props.targetId}/${props.targetType}/variable`, ids);
  loading.value = false;
  if (error) {
    return;
  }
...
const toDelete = async (data: VariableItem) => {
  const id = data.id;
  loading.value = true;
  const [error] = await http.del(`${TESTER}/target/${props.targetId}/${props.targetType}/variable`, [id], { dataType: true });
  loading.value = false;
  if (error) {
    return;
  }
...
const loadData = async () => {
  loading.value = true;
  const [error, res] = await http.get(`${TESTER}/target/${props.targetId}/${props.targetType}/variable`);
  loading.value = false;
  loaded.value = true;
  if (error) {
    return;
