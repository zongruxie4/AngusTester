...
export interface Props {
  ...
  replaceTabPane: (key: string, data: { _id: string; name: string; value: 'Ftp', scenarioInfo: { id: string; } }) => any;
}
...
const buttonGroupClick = async (data: ButtonGroupMenuItem) => {
  ...
  if (key === 'debug') {
    ...
    if (!scenarioConfigData.value?.id) {
      notification.info(t('jmsPlugin.notifications.debugSaveFirst'));
    ...
    notification.success({
      duration: 1.3,
      description: t('jmsPlugin.notifications.autoSaveSuccess')
    });
    toDebug();
    return;
  }
...
const createTest = async () => {
  ...
  notification.success(t('jmsPlugin.notifications.createTestSuccess'));
};
...
const isValid = async (): Promise<boolean> => {
  if (typeof uiConfigRef.value?.isValid === 'function') {
    if (!uiConfigRef.value.isValid()) {
      notification.error(t('jmsPlugin.notifications.taskConfigError'));
      return false;
    }
  }
  ...
  if (typeof executeConfigRef.value?.isValid === 'function') {
    ...
    if (!validFlag) {
      notification.error(t('jmsPlugin.notifications.executeConfigError'));
      return false;
    }
  }
...
const toFollow = async (id: string) => {
  ...
  notification.success(t('jmsPlugin.notifications.followSuccess'));
};
...
const cancelFollow = async (id: string) => {
  ...
  notification.success(t('jmsPlugin.notifications.cancelFollowSuccess'));
};
...
const toFavourite = async (id: string) => {
  ...
  notification.success(t('jmsPlugin.notifications.favouriteSuccess'));
};
...
const cancelFavourite = async (id: string) => {
  ...
  notification.success(t('jmsPlugin.notifications.cancelFavouriteSuccess'));
};

const authFlagChange = (_authFlag: boolean) => {
...
const save = async (data?: {
  ...
  if (!validFlag) {
    return new Error(t('jmsPlugin.notifications.paramError'));
  }
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: scenarioId, name: params.name, value: 'Ftp', scenarioInfo: { id: scenarioId } });
    }
  ...
  if (params.name !== scenarioConfigData.value.name) {
    ...
    refreshFlag = true;
  } else if (params.dirId !== scenarioConfigData.value.dirId) {
    refreshFlag = true;
  }
  if (refreshFlag) {
    // 刷新已经打开的场景列表
...
const cleanupVariables = (data: { [key: string]: any }[]) => {
  const variablesKeys: readonly string[] = ['name', 'description', 'enabled', 'scope', 'source', 'value', 'extraction'];
  const extractionKeys: readonly string[] = ['apisId', 'method', 'expression', 'defaultValue', 'location', 'parameterName', 'request', 'name'];
      ...
      for (const _key in _item) {
        if (_key === 'extraction') {
          ...
          for (const _childKey in extraction) {
            if (!extractionKeys.includes(_childKey)) {
...
const tabText = computed(() => {
  return isPageViewMode.value ? { task: t('jmsPlugin.taskConfig'), execute: t('jmsPlugin.executeConfig') } : { task: '', execute: '' };
});

const drawerMenuItems = computed(() => {
  if (scenarioConfigData.value?.id) {
    if (scenarioConfigData.value.plugin === 'Ftp') {
  ...
  <Spin :spinning="loading || !rendered" class="h-full overflow-hidden text-3 pt-1.5">
    ...
    <div
      ...
      <Drawer
        ...
        <template #activity>
          <ActivityTimeline :id="scenarioConfigData?.id" class="pt-4.75 pr-3.5" />
        </template>
    ...
    <AsyncComponent :visible="authVisible">
      <AuthorizeModal
        ...
        :onTips="t('jmsPlugin.authorizeModal.onTips')"
        :offTips="t('jmsPlugin.authorizeModal.offTips')"
        :title="t('jmsPlugin.authorizeModal.title')"
