...
const selectScriptOk = (data: ScenarioConfig['script']) => {
  ...
  if (hideButtonSet.value.has('pageView')) {
    ...
    if (codeConfigRef.value) {
      ...
      const newPipelines = (data?.task?.pipelines || []).map((i, idx) => {
        const defaultData = uiConfigRef.value.generateEmptSmtpData();
...
const save = async (data?: {
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: scenarioId, name: params.name, value: 'Mail', scenarioInfo: { id: scenarioId } });
    }
...
const generateMAIL = (): PipelineConfig => {
  return {
    id: utils.uuid(),
    target: 'MAIL',
    name: '',
...
const tabText = computed(() => {
  return isPageViewMode.value ? { task: t('mailPlugin.taskConfig'), execute: t('mailPlugin.executeConfig') } : { task: '', execute: '' };
});

const drawerMenuItems = computed(() => {
  if (scenarioConfigData.value?.id) {
    if (scenarioConfigData.value.plugin === 'Mail') {
...
<template>
  <Spin :spinning="loading || !rendered" class="h-full overflow-hidden text-3 pt-1.5">
    ...
    <AsyncComponent :visible="selectModalVisible">
      <SelectScriptModal v-model:visible="selectModalVisible" plugin="Mail" @ok="selectScriptOk" />
    </AsyncComponent>
