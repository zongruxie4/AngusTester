...
export interface Props {
  ...
  replaceTabPane: (key: string, data: { _id: string; name: string; value: 'Ldap', scenarioInfo: { id: string; } }) => any;
}
...
const buttonGroupClick = async (data: ButtonGroupMenuItem) => {
  ...
  if (key === 'debug') {
    ...
    if (!scenarioConfigData.value?.id) {
      notification.info(t('ldapPlugin.notifications.debugSaveFirst'));
    ...
    notification.success({
      duration: 1.3,
      description: t('ldapPlugin.notifications.autoSaveSuccess')
    });
    toDebug();
    return;
  }
...
const isValid = async (): Promise<boolean> => {
  if (typeof uiConfigRef.value?.isValid === 'function') {
    if (!uiConfigRef.value.isValid()) {
      notification.error(t('ldapPlugin.notifications.taskConfigError'));
      return false;
    }
  }
  ...
  if (typeof executeConfigRef.value?.isValid === 'function') {
    ...
    if (!validFlag) {
      notification.error(t('ldapPlugin.notifications.executeConfigError'));
      return false;
    }
  }
...
const toFollow = async (id: string) => {
  ...
  notification.success(t('ldapPlugin.notifications.followSuccess'));
};
...
const cancelFollow = async (id: string) => {
  ...
  notification.success(t('ldapPlugin.notifications.cancelFollowSuccess'));
};
...
const toFavourite = async (id: string) => {
  ...
  notification.success(t('ldapPlugin.notifications.favouriteSuccess'));
};
...
const cancelFavourite = async (id: string) => {
  ...
  notification.success(t('ldapPlugin.notifications.cancelFavouriteSuccess'));
};
...
const save = async (data?: {
  ...
  if (!validFlag) {
    return new Error(t('ldapPlugin.notifications.paramError'));
  }
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: scenarioId, name: params.name, value: 'Ldap', scenarioInfo: { id: scenarioId } });
    }
...
const drawerMenuItems = computed(() => {
  if (scenarioConfigData.value?.id) {
    if (scenarioConfigData.value.plugin === 'Ldap') {
  ...
  <Spin :spinning="loading || !rendered" class="h-full overflow-hidden text-3 pt-1.5">
    ...
    <AsyncComponent :visible="authVisible">
      <AuthorizeModal
        ...
        :onTips="t('ldapPlugin.authorizeModal.onTips')"
        :offTips="t('ldapPlugin.authorizeModal.offTips')"
        :title="t('ldapPlugin.authorizeModal.title')" />
    </AsyncComponent>
    ...
    <AsyncComponent :visible="selectModalVisible">
      <SelectScriptModal v-model:visible="selectModalVisible" plugin="Ldap" @ok="selectScriptOk" />
    </AsyncComponent>
