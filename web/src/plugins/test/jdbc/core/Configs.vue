...
export interface Props {
  ...
  replaceTabPane: (key: string, data: { _id: string; name: string; value: 'Jdbc', scenarioInfo: { id: string; } }) => any;
}
...
const buttonGroupClick = async (data: ButtonGroupMenuItem) => {
  ...
  if (key === 'debug') {
    ...
    if (!scenarioConfigData.value?.id) {
      notification.info(t('jdbcPlugin.notifications.debugSaveFirst'));
    ...
    notification.success({
      duration: 1.3,
      description: t('jdbcPlugin.notifications.autoSaveSuccess')
    });
    toDebug();
    return;
  }
...
const isValid = async (): Promise<boolean> => {
  if (typeof uiConfigRef.value?.isValid === 'function') {
    if (!uiConfigRef.value.isValid()) {
      notification.error(t('jdbcPlugin.notifications.taskConfigError'));
      return false;
    }
  }
  ...
  if (typeof executeConfigRef.value?.isValid === 'function') {
    ...
    if (!validFlag) {
      notification.error(t('jdbcPlugin.notifications.executeConfigError'));
      return false;
    }
  }
...
const toFollow = async (id: string) => {
  ...
  notification.success(t('jdbcPlugin.notifications.followSuccess'));
};
...
const cancelFollow = async (id: string) => {
  ...
  notification.success(t('jdbcPlugin.notifications.cancelFollowSuccess'));
};
...
const toFavourite = async (id: string) => {
  ...
  notification.success(t('jdbcPlugin.notifications.favouriteSuccess'));
};
...
const cancelFavourite = async (id: string) => {
  ...
  notification.success(t('jdbcPlugin.notifications.cancelFavouriteSuccess'));
};
...
const save = async (data?: {
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: scenarioId, name: params.name, value: 'Jdbc', scenarioInfo: { id: scenarioId } });
    }
...
const tabText = computed(() => {
  return isPageViewMode.value ? { task: t('jdbcPlugin.taskConfig'), execute: t('jdbcPlugin.executeConfig') } : { task: '', execute: '' };
});

const drawerMenuItems = computed(() => {
  if (scenarioConfigData.value?.id) {
    if (scenarioConfigData.value.plugin === 'Jdbc') {
  ...
  <Spin :spinning="loading || !rendered" class="h-full overflow-hidden text-3 pt-1.5">
    ...
    <AsyncComponent :visible="authVisible">
      <AuthorizeModal
        ...
        :onTips="t('jdbcPlugin.authorizeModal.onTips')"
        :offTips="t('jdbcPlugin.authorizeModal.offTips')"
        :title="t('jdbcPlugin.authorizeModal.title')" />
    </AsyncComponent>
    ...
    <AsyncComponent :visible="selectModalVisible">
      <SelectScriptModal v-model:visible="selectModalVisible" plugin="Jdbc" @ok="selectScriptOk" />
    </AsyncComponent>
