...
import { allCvsKeys, allColumns, throughputCvsKeys, throughputOptions, throughputColumns, threadCvsKeys, threadOptions, threadColumns, responseTimeCvsKeys, responseTimeOptions, responseTimeColumns, errorOptions, errorCvsKeys, errorColumns, oneApiErrorColumns } from '@/plugins/test/components/performanceTestDetailComp/ChartConfig';
...
const showErrCount = computed(() => {
  return +props.execInfo.sampleSummaryInfo?.errors > 0 && props.execInfo.configuration?.onError?.sampleError;
});
      ...
      <div
        ...
        <div
          ...
          <template v-if="item.key === 'qps'">
            {{ getNumFixed(props.execInfo?.sampleSummaryInfo?.ops) }}
          </template>
          ...
          <template v-if="item.key === 'export'">
            <span>{{ formatBytes(props.execInfo?.sampleSummaryInfo?.brps)? `${formatBytes(props.execInfo?.sampleSummaryInfo?.brps)}/秒`:'--' }}</span>
            <em class="not-italic inline-block w-0.5 h-3.5 mx-2 rounded" :style="{ 'background-color': 'rgb(' + item.color + ')'}" />
          ...
          <template v-if="isLoaded">
            <template v-if="timestampData.length > 0">
              <CountTemplate
                ...
                :timestampData="timestampData"
                :apiNames="apiNames"
                :cvsKeys="props.execInfo?.singleTargetPipeline?throughputCvsKeys.filter(item => item !== 'ops') : throughputCvsKeys"
          ...
          <template v-if="isLoaded">
            <template v-if="timestampData.length > 0">
              <div class="min-h-full">
                ...
                <template v-if="showErrCount">
                  <ConterList class="mt-1.5" :list="props.errCountList" />
                  <Sampling class="mt-3.5" :sampleList="props.sampleList" />
                </template>
              </div>
            </template>
            <template v-else>
