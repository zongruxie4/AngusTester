...
export interface Props {
  ...
  replaceTabPane: (key: string, data: { _id: string; name: string; value: 'Mobile', scenarioInfo: { id: string; } }) => any;
}
...
const save = async (data?: {
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: scenarioId, name: params.name, value: 'Mobile', scenarioInfo: { id: scenarioId } });
    }
...
const drawerMenuItems = computed(() => {
  ...
  if (scenarioConfigData.value?.id) {
    if (scenarioConfigData.value.plugin === 'Mobile') {
      return baseItems;
    }
