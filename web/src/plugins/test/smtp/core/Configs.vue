...
export interface Props {
  ...
  replaceTabPane: (key: string, data: { _id: string; name: string; value: 'Smtp', scenarioInfo: { id: string; } }) => any;
}
...
const scenarioConfigData = ref<ScenarioConfig>();// {{ t('smtpPlugin.comments.scenarioConfigData') }}
...
const selectScriptOk = (data: ScenarioConfig['script']) => {
  ...
  if (hideButtonSet.value.has('pageView')) {
    ...
    const newPipelines: PipelineConfig[] = (data?.task?.pipelines || []).map((i, idx) => ({ ...i, enabled: oldPipelines.length ? false : idx === 0 }));
    const pipelines = [...oldPipelines, ...newPipelines].map(item => {
      const defaultData = uiConfigRef.value.generateEmptSmtpData();
      if (item?.server?.readTimeout) {
        item.server.readTimeout = item.server.readTimeout.toLowerCase();
      }
      if (item?.server?.connectTimeout) {
        item.server.connectTimeout = item.server.connectTimeout.toLowerCase();
      }
      return {
        ...defaultData,
        ...item,
    ...
    if (codeConfigRef.value) {
      ...
      const newPipelines: PipelineConfig[] = (data?.task?.pipelines || []).map((i, idx) => {
        const defaultData = uiConfigRef.value.generateEmptSmtpData();
        if (i?.server?.readTimeout) {
          i.server.readTimeout = i.server.readTimeout.toLowerCase();
        }
        if (i?.server?.connectTimeout) {
          i.server.connectTimeout = i.server.connectTimeout.toLowerCase();
        }
        return { ...defaultData, ...i, enabled: oldPipelines.length ? false : idx === 0 };
      });
...
const save = async (data?: {
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: scenarioId, name: params.name, value: 'Smtp', scenarioInfo: { id: scenarioId } });
    }
...
const addGlobalVariables = (variableList = []) => {
  if (!scriptConfig.value.configuration) {
    scriptConfig.value.configuration = { variables: [] };
  } else if (!scriptConfig.value.configuration.variables) {
    scriptConfig.value.configuration.variables = [];
  }
  ...
  if (typeof executeConfigRef.value?.getData === 'function') {
    const configData = executeConfigRef.value?.getData();
    _variables = configData.configuration?.variables;
  } else {
    _variables = scriptConfig.value.configuration.variables;
  }

  const names = _variables.map(item => item.name);
  // 过滤重复数据
  for (let i = 0, len = variableList.length; i < len; i++) {
    if (!names.includes(variableList[i].name)) {
      _variables.unshift(variableList[i]);
    }
  }
  scriptConfig.value.configuration.variables = _variables;
};
...
const generateHTTP = (): PipelineConfig => {
  return {
    id: utils.uuid(),
    target: 'SMTP',
    name: '',
...
const drawerMenuItems = computed(() => {
  if (scenarioConfigData.value?.id) {
    if (scenarioConfigData.value.plugin === 'Smtp') {
...
<template>
  <Spin :spinning="loading || !rendered" class="h-full overflow-hidden text-3 pt-1.5">
        ...
        <div
          ...
          <AsyncComponent :visible="activeKey === 'taskConfig'">
            <UIConfig
              ...
              @addGlobalVariables="addGlobalVariables" />
          </AsyncComponent>
    ...
    <AsyncComponent :visible="selectModalVisible">
      <SelectScriptModal v-model:visible="selectModalVisible" plugin="Smtp" @ok="selectScriptOk" />
    </AsyncComponent>
