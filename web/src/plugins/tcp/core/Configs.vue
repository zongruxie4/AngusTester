...
export interface Props {
  ...
  replaceTabPane: (key: string, data: { _id: string; name: string; value: 'Tcp', sceneInfo: { id: string; } }) => any;
}
...
const save = async (data?: {
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: sceneId, name: params.name, value: 'Tcp', sceneInfo: { id: sceneId } });
    }
...
const drawerMenuItems = computed(() => {
  if (sceneConfigData.value?.id) {
    if (sceneConfigData.value.plugin === 'Tcp') {
