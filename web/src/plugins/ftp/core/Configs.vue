...
const selectScriptOk = (data: SceneConfig['script']) => {
  ...
  if (hideButtonSet.value.has('UIView')) {
    ...
    sceneConfigData.value.script.task.pipelines = cloneDeep(pipelines);
    ...
    if (codeConfigRef.value) {
      if (!codeConfigRef.value?.isValid()) {
        // 替换为新脚本
        sceneConfigData.value.script = cloneDeep(data);
        return;
      }
      ...
      if (!scriptInfo) {
        ...
        sceneConfigData.value.script = cloneDeep(scriptInfo);
      }
      ...
      if (!Object.prototype.hasOwnProperty.call(sceneConfigData.value.script, 'configuration')) {
        sceneConfigData.value.script.configuration = cloneDeep(data.configuration);
      }
...
const scriptTypeChange = (value: ScriptType) => {
  ...
  if (typeof executeConfigRef.value?.getData === 'function') {
    const configData = executeConfigRef.value.getData();
    sceneConfigData.value.script.configuration = cloneDeep(configData.configuration);
    sceneConfigData.value.script.task.arguments = cloneDeep(configData.arguments);
  }
  ...
  scriptConfig.value.configuration = cloneDeep(sceneConfigData.value.script.configuration);
  scriptConfig.value.task.arguments = cloneDeep(sceneConfigData.value.script.task.arguments);
};
...
const save = async (data?: {
  ...
  const {
    ...
    description,
    name
  } = cloneDeep(sceneConfigData.value);
