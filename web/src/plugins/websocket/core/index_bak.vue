...
import { Indicator } from 'angus-design';
...
import { WebsocketConfig, WebsocketInfo } from './UIConfig/WebSocketConfigs/PropsType';
...
const buttonGroupClick = async (data: ButtonGroupMenuItem) => {
  ...
  if (key === 'codeView') {
    if (typeof uiConfigRef.value?.getData === 'function') {
      const pipelines: WebsocketConfig[] = uiConfigRef.value.getData();
      ...
      if (typeof executeConfigRef.value?.getData === 'function') {
        ...
        let _configuration = configData.configuration;
        let _arguments = configData.arguments;
  ...
  if (key === 'debug') {
    ...
    notification.success('自动保存成功，正在执行调试...');
    toDebug();
    return;
  }
...
const createTest = async () => {
  ...
  notification.success('执行测试成功');
};

const selectScriptOk = (data: SceneConfig['script']) => {
  const pipelines = data?.task?.pipelines || [];
  const scriptType = data.type;
  if (hideButtonSet.value.has('UIView')) {
    if (typeof uiConfigRef.value?.getData === 'function') {
      const _pipelines: WebsocketConfig[] = uiConfigRef.value.getData() || [];
      pipelines.forEach(item => {
        item.id = utils.uuid();
      });
      sceneConfigData.value.script.task.pipelines = [..._pipelines, ...pipelines];
    }
    ...
    if (typeof codeConfigRef.value?.getData === 'function') {
      sceneConfigData.value.script = codeConfigRef.value.getData();
      if (sceneConfigData.value.script?.task?.pipelines) {
        const _pipelines = JSON.parse(JSON.stringify(sceneConfigData.value.script.task.pipelines)) || [];
        sceneConfigData.value.script.task.pipelines = [..._pipelines, ...pipelines];
      }
...
const scriptTypeChange = (value: ScriptType) => {
  ...
  if (typeof executeConfigRef.value?.getData === 'function') {
    const configData = executeConfigRef.value.getData();
    sceneConfigData.value.script.configuration = configData.configuration;
    sceneConfigData.value.script.task.arguments = configData.arguments;
  }
...
const save = async (data?: {
  ...
  params.script.task.pipelines = formData.pipelines.map(item => {
    delete item.id;
    return item;
  });
  ...
  if (!params.script.configuration) {
    params.script.configuration = {
      thread: {
        threads: '1'
      },
      onError: {
  ...
  const scriptType = params.script.type;
  if (!params.script.configuration.thread) {
    if (scriptType === 'TEST_FUNCTIONALITY') {
      params.script.configuration.thread = {
        threads: '1'
      };
    } else if (scriptType === 'TEST_PERFORMANCE') {
      params.script.configuration.thread = {
        threads: '5000',
        rampUpInterval: '1min', // 增压间隔
        rampUpThreads: '100' // 增压线程数
      };
    } else if (scriptType === 'TEST_STABILITY') {
      params.script.configuration.thread = {
        threads: '200'
      };
    }
    ...
    if (scriptType === 'TEST_FUNCTIONALITY') {
      if (!params.script.configuration.thread.threads) {
        params.script.configuration.thread.threads = '1';
      } else if (+params.script.configuration.thread.threads < 1) {
        params.script.configuration.thread.threads = '1';
      } else if (+params.script.configuration.thread.threads > 10) {
        params.script.configuration.thread.threads = '10';
      }
    } else if (scriptType === 'TEST_PERFORMANCE') {
      // @TODO 需要更新为具体配额数据
      if (!params.script.configuration.thread.threads) {
        params.script.configuration.thread.threads = '5000';
      } else if (+params.script.configuration.thread.threads < 1) {
        params.script.configuration.thread.threads = '1';
      } else if (+params.script.configuration.thread.threads > 10000) {
        params.script.configuration.thread.threads = '10000';
      }

      // @TODO 需要更新为具体配额数据
      if (!params.script.configuration.thread.rampUpInterval) {
        params.script.configuration.thread.rampUpInterval = '1min';
      }

      // @TODO 需要更新为具体配额数据
      if (!params.script.configuration.thread.rampUpThreads) {
        params.script.configuration.thread.rampUpThreads = '100';
      } else if (+params.script.configuration.thread.rampUpThreads < 1) {
        params.script.configuration.thread.rampUpThreads = '1';
      } else if (+params.script.configuration.thread.rampUpThreads > +params.script.configuration.thread.threads) {
        params.script.configuration.thread.rampUpThreads = params.script.configuration.thread.threads;
      }
    } else if (scriptType === 'TEST_STABILITY') {
      if (!params.script.configuration.thread.threads) {
        params.script.configuration.thread.threads = '200';
      } else if (+params.script.configuration.thread.threads < 1) {
        params.script.configuration.thread.threads = '1';
      } else if (+params.script.configuration.thread.threads > 10000) {
        params.script.configuration.thread.threads = '10000';
  ...
  if (!params.script.configuration.iterations) {
    params.script.configuration.iterations = '1';
  }

  if (!params.script.configuration.duration) {
    if (scriptType === 'TEST_PERFORMANCE') {
      params.script.configuration.duration = '50min';
    } else if (scriptType === 'TEST_STABILITY') {
      params.script.configuration.duration = '30min';
    }
  }
  ...
  if (!params.id) {
    ...
    if (typeof props.replaceTabPane === 'function') {
      props.replaceTabPane(props.tabKey, { _id: sceneId, name: params.name, value: 'WebSocket', sceneInfo: { id: sceneId } });
    }
  ...
  if (typeof props.updateTabPane === 'function') {
    if (typeof props.getTabPane === 'function') {
      const tabData = props.getTabPane(dirId);
      if (tabData?.length) {
        props.updateTabPane({ _id: dirId, notify: utils.uuid() });
      }
    }
  }
};

const transformPipelines = (data: WebsocketInfo[]): WebsocketConfig[] => {
  return data.map(item => {
    const id = utils.uuid();
    const assertions = item.assertions?.map(ele => {
      return {
        ...ele,
        type: ele.type?.value,
        assertionCondition: ele.assertionCondition?.value
      };
    });
    return {
      ...item,
  ...
  if (script) {
    ...
    if (script.task) {
      ...
      if (script.task.pipelines?.length) {
        sceneConfigData.value.script.task.pipelines = transformPipelines(script.task.pipelines);
      }
    }
...
onMounted(() => {
  // watch(() => executeConfigRef.value, async () => {
  //   console.log(executeConfigRef.value.getData());
  // });
  ...
  watch(() => props.sceneInfo, async (newValue) => {
    ...
    const { dirId, dirName, name } = newValue;
...
const generateWebSocket = (): WebsocketConfig => {
  return {
...
<template>
  <Spin :spinning="loading" class="h-full overflow-hidden text-3 pt-1.5">
    <Tabs
    ...
    <div
      ...
      <div class="flex-1 h-full min-w-0">
        ...
        <Toolbar
          ...
          <template #debugResult>
            <DebugResult :value="debugExecInfo" />
          </template>
          <template #logs>
      ...
      <Drawer
        ...
        <template #indicator>
          <Indicator :id="sceneConfigData?.id" type="SCENE" />
        </template>

        <template #testInfo>
          <HttpTestInfo :id="sceneConfigData?.id" type="SCENE" />
        </template>

        <template #activity>
          <ActivityTimeline :id="sceneConfigData?.id" class="pt-4.75 pr-3.5" />
        </template>
