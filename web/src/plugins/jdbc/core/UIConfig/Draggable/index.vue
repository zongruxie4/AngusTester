...
const append = (targetId: string) => {
  ...
  for (let i = 0, len = list.length; i < len; i++) {
    ...
    if (id === targetId) {
      ...
      const sourceData:PipelineConfig = dataMap.value[targetId];
      ...
      if (children?.length) {
        ...
        for (let j = 0, _len = children.length; j < _len; j++) {
          ...
          const _sourceData:PipelineConfig = dataMap.value[children[j].id];
...
const nameChange = (key:TargetKey, id:string) => {
  ...
  if (key === 'TRANS_START') {
    if (typeof refsMap.value[id]?.getName === 'function') {
      dataMap.value[id].name = refsMap.value[id]?.getName();
    }
  }
};
        ...
        <li
          ...
          <template v-else-if="target === 'WAITING_TIME'">
            <WaitingTime
              ...
              :repeatNames="repeatNames"
              @click="actionClick($event, id)"
              @nameChange="nameChange('WAITING_TIME', id)"
              @enabledChange="enabledChange($event, id)"
              @change="change($event,id)"
              @renderChange="renderChange" />
          </template>
          <template v-else-if="target === 'RENDEZVOUS'">
            <Rendezvous
              ...
              :repeatNames="repeatNames"
              @click="actionClick($event, id)"
              @nameChange="nameChange('RENDEZVOUS', id)"
              @enabledChange="enabledChange($event, id)"
              @change="change($event,id)"
              @renderChange="renderChange" />
          </template>
          <template v-else-if="target === 'TRANS_START'">
            <TransStart
              ...
              :repeatNames="repeatNames"
              @click="actionClick($event, id)"
              @nameChange="nameChange('TRANS_START', id)"
              @enabledChange="enabledChange($event, id)"
              @change="change($event,id)"
              @renderChange="renderChange">
              <template #default>
                    ...
                    <li
                      ...
                      <template v-if="_target === 'JDBC'">
                        <JDBCConfigs
                          ...
                          @nameChange="nameChange('JDBC', _cid)"
                      ...
                      <template v-else-if="_target === 'RENDEZVOUS'">
                        <Rendezvous
                          ...
                          class="mx-5"
                          @click="actionClick($event, _cid)"
                          @nameChange="nameChange('RENDEZVOUS', _cid)"
                          @enabledChange="enabledChange($event, _cid)"
                          @change="change($event,_cid)"
                          @renderChange="renderChange" />
                      </template>
