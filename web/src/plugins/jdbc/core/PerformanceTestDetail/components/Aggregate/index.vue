...
import { allResponseTimeColumns, allCvsNames, allErrorsColumns, allErrorRateColumns, allUploadColumns, throughputColumns } from '../../ChartConfig';
...
const oneApiTableKey = ref(['tranMean', 'tranMin', 'tranMax', 'tranP50', 'tranP75', 'tranP90', 'tranP95', 'tranP99', 'tranP999']);
...
const setUpdateSeriesData = () => {
  ...
  if (activeKey.value === 'throughput') {
    oneApiTableKey.value = props.isSingleInterface ? ['tps', 'brps', 'bwps'] : ['ops', 'tps', 'brps', 'bwps'];
  }
...
const radioGroupChange = (e) => {
  ...
  xData.value = props.timestampData.slice(sliderValue.value[0], sliderValue.value[1] + 1);
  setUpdateSeriesData();
  tableData.value = getTableData({}, oneApiTableKey.value, {});
};
...
const columns = computed(() => {
  if (props.isSingleInterface) {
    ...
    if (activeKey.value === 'throughput') {
      return props.isSingleInterface ? throughputColumns.filter(item => item.dataIndex !== 'ops' && item.dataIndex !== 'operations') : throughputColumns;
    }
    if (activeKey.value === 'errorNum') {
