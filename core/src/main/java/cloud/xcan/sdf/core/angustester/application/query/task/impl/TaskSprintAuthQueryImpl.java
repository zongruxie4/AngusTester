...
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.TASK_SPRINT_NO_AUTH;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.TASK_SPRINT_NO_AUTH_CODE;
...
public class TaskSprintAuthQueryImpl implements TaskSprintAuthQuery {
  ...
  public Boolean status(Long sprintId) {
    return new BizTemplate<Boolean>() {
      ...
      protected Boolean process() {
        return taskSprintDb.getAuthFlag();
      }
    }.execute();
  }
  ...
  @Override
  public TaskSprintAuthCurrent currentUserAuth(Long sprintId, Boolean adminFlag) {
    return new BizTemplate<TaskSprintAuthCurrent>() {
      ...
      protected TaskSprintAuthCurrent process() {
        ...
        authCurrent.setTaskSprintAuthFlag(sprintDb.getAuthFlag());
  ...
  public void batchCheckPermission(Collection<Long> sprintIds, TaskSprintPermission permission) {
    ...
    Collection<Long> authIds = permission.isGrant()
        ? sprintIds : taskSprintRepo.findIds0ByIdInAndAuthFlag(sprintIds, true);
    if (isEmpty(authIds)) {
      return;
    }
