...
public interface TaskInfoRepo extends BaseRepository<TaskInfo, Long>,
  ...
  @Query(value = "SELECT * FROM task WHERE sprint_id = ?1 AND assignee_id = ?2 AND deleted_flag = 0 AND sprint_deleted_flag =0", nativeQuery = true)
  List<TaskInfo> findAllBySprintIdAndAssigneeId(Long sprintId, Long userId);

  @Query(value = "SELECT * FROM task WHERE project_id = ?1 AND assignee_id = ?2 AND deleted_flag = 0 AND sprint_deleted_flag =0", nativeQuery = true)
  ...
  @Query(value = "SELECT * FROM task WHERE project_id = ?1 AND deleted_flag = 0 AND sprint_deleted_flag = 0 ORDER BY created_date ASC LIMIT 1", nativeQuery = true)
  TaskInfo findEarliestByProjectId(Long projectId);

  @Query(value = "SELECT * FROM task WHERE sprint_id = ?1 AND deleted_flag = 0 AND sprint_deleted_flag = 0 ORDER BY created_date ASC LIMIT 1", nativeQuery = true)
  ...
  @Query(value =
      "SELECT * FROM task WHERE overdue_flag = 0 AND ((deadline_date < now() AND STATUS <> 'COMPLETED' AND STATUS <> 'CANCELED') "
          + "OR (completed_date > deadline_date AND STATUS = 'COMPLETED' )) LIMIT ?1", nativeQuery = true)
  List<TaskInfo> findIdByOverdue(Long count);

  @Transactional
  @Modifying
  @Query(value = "UPDATE task SET overdue_flag=1 WHERE id IN ?1", nativeQuery = true)
