...
import static cloud.xcan.sdf.core.angustester.domain.TesterEventMessage.FunctionCaseWillOverdue;
import static cloud.xcan.sdf.core.angustester.domain.TesterEventMessage.FunctionCaseWillOverdueCode;
...
public class FuncCaseWillOverdueEventJob {
  ...
  public void execute() {
    ...
    try {
      ...
      if (!tryLock) {
        log.warn("FuncCaseWillOverdueEventJob try lock fail!");
        return;
      }
      ...
      if (ObjectUtils.isNotEmpty(funcCases)) {
        assembleAndSendOverdueNoticeEvent(funcCases);
      }
      log.debug("FuncCaseWillOverdueEventJob execute successfully");
    } catch (Exception e) {
      log.error("FuncCaseWillOverdueEventJob execute fail", e);
    } finally {
