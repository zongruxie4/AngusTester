...
import static cloud.xcan.sdf.api.commonlink.setting.quota.QuotaResource.AngusTesterTag;
...
public class TagQueryImpl implements TagQuery {
  ...
  public Page<Tag> find(GenericSpecification<Tag> spec, Pageable pageable) {
    return new BizTemplate<Page<Tag>>() {
      ...
      protected Page<Tag> process() {
        ...
        setEditPermissionFlag(spec.getCriterias(), page.getContent());
        return page;
      }
    }.execute();
  }
  ...
  @Override
  public void setEditPermissionFlag(Set<SearchCriteria> criteria, List<Tag> tags) {
    if (ObjectUtils.isEmpty(tags)) {
      return;
    }
