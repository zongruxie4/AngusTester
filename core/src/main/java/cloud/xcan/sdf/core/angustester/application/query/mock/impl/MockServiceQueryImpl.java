...
import static cloud.xcan.sdf.api.commonlink.TesterConstant.MOCK_SERVICE_CLOUD_DOMAIN_SUFFIX;
import static cloud.xcan.sdf.api.commonlink.setting.quota.QuotaResource.AngusTesterMockServiceApis;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_ASSOC_SERVICE_EXISTED_T;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_DOMAIN_IN_USE_T;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_DOMAIN_PORT_IN_USE_T;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_NAME_EXISTED_T;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_NODE_PORT_IN_USE_T;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_OVER_LIMIT_IN_NODE;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_PORT_UNAVAILABLE_IN_AGENT_T;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.MOCK_SERVICE_TOP_LEVEL_DOMAIN_ERROR_T;
...
import cloud.xcan.sdf.api.angusctrl.node.dto.NodeAgentCheckPortDto;
import cloud.xcan.sdf.api.angusctrl.mockservice.dto.MockServiceStatusDto;
...
import cloud.xcan.sdf.core.angustester.domain.mock.service.MockServiceAssocP;
...
public class MockServiceQueryImpl implements MockServiceQuery {
  ...
  public MockResourcesCreationCount creationStatistics(Long projectId,
    ...
    return new BizTemplate<MockResourcesCreationCount>() {
      ...
      protected MockResourcesCreationCount process() {
        ...
        allFilters.add(SearchCriteria.equal("pushbackFlag", 1));
  ...
  public void checkNodeServiceNum(long nodeId, long existedNodeNum) {
    if (existedNodeNum >= 2) {
      ...
      if (nonNull(detailVo.getInfo())
        ...
        throw CommProtocolException.of(MOCK_SERVICE_OVER_LIMIT_IN_NODE);
      }
    }
  }
  ...
  public void checkNodeAndPortAvailable(Node nodeDb, int port) {
    ...
    if (!result.isSuccess()) {
      throw CommProtocolException.of(MOCK_SERVICE_PORT_UNAVAILABLE_IN_AGENT_T,
  ...
  public void setMockServiceCurrentAuths(List<MockService> mockServices) {
    if (isNotEmpty(mockServices)) {
      ...
      for (MockService service : mockServices) {
        if (!service.getAuthFlag() || commonQuery.isAdminUser()) {
  ...
  public Boolean isAuthCtrl(Long id) {
    Boolean authFlag = mockServiceRepo.findAuthFlagById(id);
