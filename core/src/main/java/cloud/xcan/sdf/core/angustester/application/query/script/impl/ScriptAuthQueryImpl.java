...
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.SCRIPT_NO_AUTH_CODE;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.SCRIPT_NO_AUTH_T;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.SCRIPT_NO_TARGET_AUTH;
import static cloud.xcan.sdf.core.angustester.domain.TesterCoreMessage.SCRIPT_NO_TARGET_AUTH_CODE;
...
public class ScriptAuthQueryImpl implements ScriptAuthQuery {
  ...
  @Override
  public ScriptAuthCurrent currentUserAuth(Long scriptId, Boolean adminFlag) {
    return new BizTemplate<ScriptAuthCurrent>() {
      ...
      protected ScriptAuthCurrent process() {
        ...
        authCurrent.setScriptAuthFlag(scriptDb.getAuthFlag());
  ...
  @Override
  public Map<Long, ScriptAuthCurrent> currentUserAuths(HashSet<Long> scriptIds, Boolean adminFlag) {
    return new BizTemplate<Map<Long, ScriptAuthCurrent>>() {
      ...
      protected Map<Long, ScriptAuthCurrent> process() {
        ...
        if (nonNull(adminFlag) && adminFlag && commonQuery.isAdminUser()) {
          for (ScriptInfo script : scriptsDb) {
            ScriptAuthCurrent authCurrent = new ScriptAuthCurrent();
            authCurrent.setScriptAuthFlag(script.getAuthFlag());
            authCurrent.addPermissions(ScriptPermission.ALL);
  ...
  public void batchCheckPermission(Collection<Long> scriptIds, ScriptPermission permission) {
    ...
    Collection<Long> authIds = permission.isGrant()
        ? scriptIds : scriptRepo.findIds0ByIdInAndAuthFlag(scriptIds, true);
    if (isEmpty(authIds)) {
      return;
    }
