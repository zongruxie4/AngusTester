...
public interface ApisRepo extends BaseRepository<Apis, Long> {
  ...
  @Query(value = "SELECT service_id FROM apis WHERE id in ?1 AND deleted_flag = 0 AND service_deleted_flag = 0", nativeQuery = true)
  ...
  @Query(value = "SELECT id FROM apis WHERE service_id in ?1 AND deleted_flag = 0 AND service_deleted_flag = 0", nativeQuery = true)
  List<Long> findAllIdByServiceIdIn(List<Long> serviceIds);

  @Query(value = "SELECT id FROM apis WHERE service_id = ?1 AND id IN ?2 AND deleted_flag = 0 AND service_deleted_flag = 0", nativeQuery = true)
  List<Long> findAllIdByServiceIdAndIdIn(Long serviceId, Collection<Long> apisIds);

  @Query(value = "SELECT * FROM apis WHERE service_id = ?1 AND id IN ?2 AND deleted_flag = 0 AND service_deleted_flag = 0", nativeQuery = true)
  ...
  @Query(value = "SELECT DISTINCT service_id FROM apis WHERE service_id IN ?1 AND deleted_flag = 0", nativeQuery = true)
  ...
  @Modifying
  @Query(value = "UPDATE apis a SET a.auth_flag=?2 WHERE a.id=?1", nativeQuery = true)
  void updateAuthFlagById(Long id, Boolean enabledFlag);

  @Modifying
  @Query(value = "UPDATE apis a SET a.service_auth_flag=?2 WHERE a.service_id=?1", nativeQuery = true)
  void updateServiceAuthFlagByServiceId(Long serviceId, Boolean enabledFlag);
  ...
  @Modifying
  @Query(value = "UPDATE apis a SET a.deleted_flag=?2, a.deleted_by =?3, a.deleted_date = ?4 WHERE a.id in ?1", nativeQuery = true)
  void updateDeleteStatus(Collection<Long> ids, Boolean deletedFlag, Long deletedBy,
      LocalDateTime deletedDate);

  @Modifying
  @Query(value = "UPDATE apis a SET a.service_deleted_flag=?2 WHERE a.service_id in ?1", nativeQuery = true)
  void updateServiceDeleteStatusByService(Collection<Long> serviceIds, boolean deletedFlag);

  @Modifying
  @Query(value = "UPDATE apis a SET a.deleted_flag = 0, a.deleted_by = -1, a.deleted_date = null WHERE a.id in ?1", nativeQuery = true)
  ...
  @Modifying
  @Query(value = "UPDATE apis a SET a.test_func_flag =?2 WHERE a.service_id = ?1", nativeQuery = true)
  void updateTestFuncFlagByService(Long serviceId, Boolean enabled);

  @Modifying
  @Query(value = "UPDATE apis a SET a.test_perf_flag =?2 WHERE a.service_id = ?1", nativeQuery = true)
  void updateTestPerfFlagByService(Long serviceId, Boolean enabled);

  @Modifying
  @Query(value = "UPDATE apis a SET a.test_stability_flag =?2 WHERE a.service_id = ?1", nativeQuery = true)
  void updateTestStabilityFlagByService(Long serviceId, Boolean enabled);
