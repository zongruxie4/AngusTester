...
public interface TaskSprintAuthRepo extends BaseRepository<TaskSprintAuth, Long> {
  ...
  @Transactional
  @Modifying
  @Query(value = "DELETE FROM task_sprint_auth WHERE sprint_id = ?1 AND auth_object_id IN ?2 AND creator_flag = ?3", nativeQuery = true)
  void deleteBySprintIdAndAuthObjectIdInAndCreatorFlag(Long sprintId, Set<Long> creatorIds,
      boolean creatorFlag);
