...
import static cloud.xcan.sdf.core.angustester.application.converter.KanbanCtoTaskConverter.calcProcessedDays;
import static cloud.xcan.sdf.core.angustester.application.query.kanban.impl.KanbanDataAssetsQueryImpl.getTimeSeriesByFormat;
import static cloud.xcan.sdf.core.angustester.domain.kanban.AssigneeRanking.rank;
import static cloud.xcan.sdf.core.angustester.domain.task.TaskStatus.CANCELED;
import static cloud.xcan.sdf.core.angustester.domain.task.TaskStatus.COMPLETED;
import static cloud.xcan.sdf.core.angustester.domain.task.TaskStatus.CONFIRMING;
import static cloud.xcan.sdf.core.angustester.domain.task.TaskStatus.IN_PROGRESS;
import static cloud.xcan.sdf.core.angustester.domain.task.TaskStatus.PENDING;
...
public class KanbanEfficiencyTaskConverter {
  ...
  public static void assembleOverdueOverview(List<? extends TaskEfficiencySummary> tasks,
    ...
    totalOverview.setOverdueNum(tasks.stream().filter(x -> !x.getStatus().isCanceled()
        && nonNull(x.getOverdueFlag()) && x.getOverdueFlag()).count());
  ...
  public static CoreKpiCount assembleTaskCoreKpiOverview(
    ...
    count.setOverdueNum(tasks.stream().filter(
            x -> nonNull(x.getOverdueFlag()) && x.getOverdueFlag() && !x.getStatus().isCanceled())
        .count());
    count.setOverdueRate(calcRate(count.getOverdueNum(), count.getTotalNum()));
    count.setCompletedOverdueNum(tasks.stream().filter(
            x -> nonNull(x.getOverdueFlag()) && x.getOverdueFlag() && x.getStatus().isCompleted())
        .count());
  ...
  public static void assembleBugCount(List<? extends TaskEfficiencySummary> tasks,
    ...
    if (isNotEmpty(validBugs)) {
      ...
      totalOverview.setMissingBugNum(validBugs.stream()
          .filter(x -> nonNull(x.getMissingBugFlag()) && x.getMissingBugFlag()).count());
  ...
  public static void assembleFailure(List<? extends TaskEfficiencySummary> tasks,
    ...
    if (isNotEmpty(validBugs)) {
      ...
      totalOverview.setFailureOverdueNum(
          validBugs.stream().filter(x -> nonNull(x.getOverdueFlag()) && x.getOverdueFlag())
              .count());
  ...
  public static void assembleUnplanned(List<? extends TaskEfficiencySummary> tasks,
    ...
    List<? extends TaskEfficiencySummary> unplannedTasks = tasks.stream()
        .filter(x -> !x.getStatus().isCanceled() && !x.getTaskType().isBug()
            && x.getUnplannedFlag()).collect(Collectors.toList());
  ...
  public static ResourceCreationCount assembleResourceCreationCount(
    ...
    total.getTimeSeries().put("TASK", getTimeSeriesByFormat(
        tasks.stream().filter(x -> nonNull(x.getBacklogFlag()) && !x.getBacklogFlag())
            .collect(Collectors.toList()), DEFAULT_DAY_FORMAT));
