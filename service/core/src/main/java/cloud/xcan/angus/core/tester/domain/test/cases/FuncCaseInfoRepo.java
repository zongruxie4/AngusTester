...
public interface FuncCaseInfoRepo extends BaseRepository<FuncCaseInfo, Long> {
  ...
  @Query(value = "SELECT id FROM func_case WHERE project_id = ?1", nativeQuery = true)
  ...
  @Query(value =
      "SELECT * FROM func_case WHERE overdue = 0 AND ((deadline_date < now() AND test_result <> 'PASSED' AND test_result <> 'CANCELED') "
  ...
  @Query(value = "SELECT * FROM func_case WHERE project_id = ?1 AND deleted = 0 AND plan_deleted = 0 ORDER BY created_date ASC LIMIT 1", nativeQuery = true)
  FuncCaseInfo findEarliestByProjectId(Long projectId);

  @Query(value = "SELECT * FROM func_case WHERE plan_id = ?1 AND deleted = 0 AND plan_deleted = 0 ORDER BY created_date ASC LIMIT 1", nativeQuery = true)
  FuncCaseInfo findEarliestByPlanId(Long planId);

  @Query(value = "SELECT * FROM func_case WHERE plan_id = ?1 AND deleted = 0 AND plan_deleted = 0 ORDER BY created_date DESC LIMIT 1", nativeQuery = true)
  ...
  @Query(value = "SELECT * FROM func_case WHERE deadline_date < ?1 AND deadline_date >= ?2 AND test_result <> 'PASSED' AND test_result <> 'CANCELED' AND tester_id <> null AND deleted = 0 AND plan_deleted = 0 ORDER BY created_date ASC LIMIT ?3", nativeQuery = true)
