package cloud.xcan.angus.core.tester.domain.scenario.follow;
...
public interface ScenarioFollowRepo extends BaseRepository<ScenarioFollow, Long> {
  ...
  @Query(value =
      "select count(af.id) FROM scenario_follow af, scenario s WHERE s.id = af.scenario_id "
          + "AND af.created_by = ?1 AND s.deleted = 0", nativeQuery = true)
  Long countByCreatedBy(Long userId);

  @Query(value =
      "select count(af.id) FROM scenario_follow af, scenario s WHERE s.id = af.scenario_id "
          + "AND af.project_id = ?1 AND af.created_by = ?2 AND s.deleted = 0", nativeQuery = true)
