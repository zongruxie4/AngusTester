...
public interface ReportAuthRepo extends BaseRepository<ReportAuth, Long> {
  ...
  @Transactional
  @Modifying
  @Query(value = "DELETE FROM report_auth WHERE report_id = ?1 AND auth_object_id IN ?2 AND creator = ?3", nativeQuery = true)
  void deleteByReportIdAndAuthObjectIdInAndCreator(Long reportId, Set<Long> creatorIds,
      boolean creator);
