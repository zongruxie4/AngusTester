...
import static cloud.xcan.angus.remote.search.SearchCriteria.isNotNull;
...
import static cloud.xcan.angus.spec.utils.ObjectUtils.isTrueValue;
import static cloud.xcan.angus.spec.utils.ObjectUtils.nullSafe;
import static cloud.xcan.angus.spec.utils.ProbabilityUtils.randomWithProbability;
...
public class TaskConverter {
  ...
  public static void assembleAddTaskInfo(Task task, @Nullable TaskSprint sprintDb,
    ...
    task.setUnplanned(nonNull(sprintDb) && sprintDb.getStatus().isStarted());
    ...
    task.setSprintAuth(nonNull(sprintDb) ? sprintDb.getAuth() : false);
  ...
  public static void assembleUpdateTask(Task task, Task taskDb) {
    ...
    taskDb.setOverdue(nonNull(taskDb.getDeadlineDate())
  ...
  public static void assembleReplaceTask(Task task, Task taskDb) {
    ...
    if (task.getTaskType().isBug()) {
      taskDb.setMissingBug(task.getMissingBug());
    } else {
      taskDb.setMissingBug(false);
    }
    if (nonNull(task.getActualWorkload()) && isNull(taskDb.getEvalWorkload())) {
      taskDb.setEvalWorkload(task.getActualWorkload());
    }
  ...
  public static void assembleMoveTask(TaskSprint targetSprintDb, Task taskDb) {
    ...
    taskDb.setUnplanned(nonNull(targetSprintDb) && targetSprintDb.getStatus().isStarted());
    if (nonNull(targetSprintDb)) {
      taskDb.setProjectId(targetSprintDb.getProjectId())
          ...
          .setSprintDeleted(targetSprintDb.getDeleted())
          .setSprintAuth(targetSprintDb.getAuth())
          .setDeadlineDate(nullSafe(taskDb.getDeadlineDate(),
              targetSprintDb.getDeadlineDate()))
          .setOverdue(taskDb.getDeadlineDate().isBefore(LocalDateTime.now()));
    }
  }
  ...
  public static TaskSummary toTaskSummary(TaskInfo task) {
    ...
    summary.setBacklog(task.getBacklog());
    ...
    try {
      ...
      for (String[] row : data) {
        ...
        Task task = new Task().setId(uidGenerator.getUID())
            ...
            .setBacklog(/* Agile Project Management and not in Sprint */
            ...
            .setUnplanned(unplannedIdx != -1 && isTrueValue(row[unplannedIdx]))
      ...
      throw ProtocolException.of("Data format error: " + e.getMessage());
    }
    return tasks;
  }
  ...
  public static @NotNull TaskAssigneeCount assembleTaskAssigneeCount(Long assigneeId,
    ...
    count.setOverdueNum(
        validAssigneeTasks.stream().filter(TaskEfficiencySummary::getOverdue).count());
  ...
  public static @NotNull Set<SearchCriteria> getTaskResourcesFilter(Long projectId,
    ...
    filters.add(equal("sprintDeleted", false));
    return filters;
  }
}
